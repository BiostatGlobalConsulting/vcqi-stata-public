...
program define make_tables_from_RI_QUAL_09
	...
	local oldvcp $VCP
	global VCP make_tables_from_RI_QUAL_09
	...
	* require either lower case or upper case in this program
	local ldose `=lower("`dose'")'
	local udose `=upper("`dose'")'
	
	if "`dose'" == "anydose" local ldose anydose
	if "`dose'" == "anydose" local udose Any Dose
	
	if "`dose'" == "anydose" {
		local x1 n_partial pct_partial
		local x2 (.) (.)
		local x3 (n_partial[\`i']) (pct_partial[\`i'])
	}
	...
	if "$VCQI_DEBUG" == "1" vcqi_log_comment $VCP 3 Comment "ldose = `ldose'"
	...
	*
	...
	global `measureid'_TEMP_DATASETS ${`measureid'_TEMP_DATASETS} `measureid'_${ANALYSIS_COUNTER}_`ldose'_TO
	  
	use "${VCQI_OUTPUT_FOLDER}/`measureid'_${ANALYSIS_COUNTER}_`ldose'_database", clear
	...
	* calculate the three percent figures
	gen pct_mov = n_mov/n_eligible * 100 if n_eligible > 0
	gen pct_uncor = n_uncor_mov / n_mov * 100 if n_mov > 0
	...
	* corrected if we're building a table for all doses
	if "`dose'" == "anydose" {
		gen n_partial = n_mov - n_uncor_mov - n_cor_mov
		gen pct_partial = n_partial / n_mov * 100 if n_mov > 0
		replace pct_partial = 0 if missing(pct_partial)
	}
	...
	* simply saying `postblankrow'.
	...
	if $SHOW_BLANKS_BETWEEN_LEVELS == 1 {
		local postblankrow post to_dataset ("") (.) (.) (.) (.) (.) (.) (.) (.) (.) (.) `x2'
	}
	...
	* Do a little housecleaning and then export it to Excel.
	...
	use "${VCQI_OUTPUT_FOLDER}/`measureid'_${ANALYSIS_COUNTER}_`ldose'_TO", clear
	qui compress
	save, replace
	...
	* stratum names in the output, and start at column 1
	if "${LC4_`sheet_nospaces'}" == "" {
		if "`dose'" != "anydose" {
			local vlist stratum n_mov pct_mov n_uncor_mov pct_uncor n_cor_mov pct_cor n
			local nextcolumn 1
		...
		if "`dose'" == "anydose" {
			local vlist stratum n_mov pct_mov n_uncor_mov pct_uncor n_cor_mov pct_cor n_partial pct_partial n
			local nextcolumn 1
			local sheetoption sheetreplace
			local stratum		1
			local n_mov  		2
	...
	* N column will also show in the final worksheet
		...
		if "`dose'" != "anydose" {
			...
			local n_mov  		`nextcolumn'
			local pct_mov		`=`nextcolumn'+1'
			local n_uncor_mov	`=`nextcolumn'+2'
			local pct_uncor		`=`nextcolumn'+3'
			local n_cor_mov		`=`nextcolumn'+4'
			local pct_cor		`=`nextcolumn'+5'
			local n				`=`nextcolumn'+6'
		}
		if "`dose'" == "anydose" {
			local vlist n_mov pct_mov n_uncor_mov pct_uncor n_cor_mov pct_cor n_partial pct_partial n 
			local nextcolumn = `=${LC4_`sheet_nospaces'}+1'
			local sheetoption sheetmodify
			local n_mov  		`nextcolumn'
			local pct_mov		`=`nextcolumn'+1'
			...
			local n_cor_mov		`=`nextcolumn'+4'
			local pct_cor		`=`nextcolumn'+5'
			local n_partial		`=`nextcolumn'+6'
			local pct_partial	`=`nextcolumn'+7'
			local n				`=`nextcolumn'+8'
		}
	}
	...
	* name of the worksheet.
	...
	if "${LC4_`sheet_nospaces'}" == "" {
		vcqi_global LC4_`sheet_nospaces' 8
	}
	else {
		vcqi_global LC4_`sheet_nospaces' `=${LC4_`sheet_nospaces'} + 7'
	}	
	
	if "`dose'" == "anydose" vcqi_global LC4_`sheet_nospaces' `=${LC4_`sheet_nospaces'} + 2'
	...
	* Column labels
	mata: b.put_string(`startrow',`n_mov',      "Had MOV for `udose' (N)")
	mata: b.put_string(`startrow',`pct_mov',    "Had MOV for `udose' (%)")
	mata: b.put_string(`startrow',`n_uncor_mov',"MOV uncorrected for `udose' (N)")
	mata: b.put_string(`startrow',`pct_uncor',  "MOV uncorrected for `udose' (%)")
	mata: b.put_string(`startrow',`n_cor_mov',  "MOV corrected for `udose' (N)")
	mata: b.put_string(`startrow',`pct_cor',    "MOV corrected for `udose' (%)")
	mata: b.put_string(`startrow',`n',          "Had visits eligible for `udose' (N)")
	
	if "`dose'" == "anydose" {
		mata: b.put_string(`startrow',`n_mov',       "Had MOV for any dose (N)")
		mata: b.put_string(`startrow',`pct_mov',     "Had MOV for any dose (%)")
		mata: b.put_string(`startrow',`n_uncor_mov', "All MOVs were uncorrected (N)")
		mata: b.put_string(`startrow',`pct_uncor',   "All MOVs were uncorrected (%)")
		mata: b.put_string(`startrow',`n_cor_mov',   "All MOVs were corrected (N)")
		mata: b.put_string(`startrow',`pct_cor',     "All MOVs were corrected (%)")
		mata: b.put_string(`startrow',`n_partial',   "Some (not all) MOVs were corrected (N)")
		mata: b.put_string(`startrow',`pct_partial', "Some (not all) MOVs were corrected (%)")
		mata: b.put_string(`startrow',`n',           "Had visits eligible for any dose (N)")
	}
	
	* Export title and footnotes
	if `nextcolumn' == 1 {
	...
	if "$FORMAT_EXCEL" == "1" {
		...
		mata: b.set_number_format((`rows'),`pct_mov',  "##0.0;;0.0;")
		mata: b.set_number_format((`rows'),`pct_uncor',"##0.0;;0.0;")
		mata: b.set_number_format((`rows'),`pct_cor',  "##0.0;;0.0;")

		mata: b.set_column_width(`n_mov',`n_mov'             , 24)
		mata: b.set_column_width(`n_uncor_mov',`n_uncor_mov' , 30)
		mata: b.set_column_width(`n_cor_mov',`n_cor_mov'     , 30)
		mata: b.set_column_width(`n',`n'                     , 35)
		
		mata: b.set_column_width(`pct_mov',`pct_mov'     , 24)
		mata: b.set_column_width(`pct_uncor',`pct_uncor' , 30)
		mata: b.set_column_width(`pct_cor',`pct_cor'     , 30)
		
		mata: b.set_number_format((`rows'),`n_mov',      "number_sep")
		mata: b.set_number_format((`rows'),`n_uncor_mov',"number_sep")
		mata: b.set_number_format((`rows'),`n_cor_mov',  "number_sep")
		mata: b.set_number_format((`rows'),`n',          "number_sep")
		
		if "`dose'" == "anydose" {
			mata: b.set_number_format((`rows'),`pct_partial',  "##0.0;;0.0;")
			mata: b.set_column_width(`n_partial',`n_partial'     , 40)
			mata: b.set_column_width(`pct_partial',`pct_partial' , 40)
			mata: b.set_number_format((`rows'),`n_partial',      "number_sep")
		}
		...
		* add bold or shading or indent or italics
		forvalues j = 1/`=_N' {
			...
			* Indent level 4 stratum names unless they are headings (LABEL_ONLY) with no estimate
			if `nextcolumn' == 1 & substratum[`j'] == 1 & !missing(n_mov[`j']) ///
	...
	vcqi_log_comment $VCP 3 Comment "Tabular output: `measureid' - `udose' to sheet `sheet' in ${VCQI_OUTPUT_FOLDER}/${VCQI_ANALYSIS_NAME}_TO.xlsx"	
