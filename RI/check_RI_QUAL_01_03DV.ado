*! check_RI_QUAL_01_03DV version 1.00 - Biostat Global Consulting - 2020-12-09
*******************************************************************************
* Change log
* 				Updated
*				version
* Date 			number 	Name			What Changed
* 2020-12-09	1.00	Dale Rhoda		Original version from check_RI_COVG_02_03DV
*******************************************************************************
...
program define check_RI_QUAL_01_03DV
	version 14.1
	
	local oldvcp $VCP
	global VCP check_RI_QUAL_01_03DV
	vcqi_log_comment $VCP 5 Flow "Starting"	

	local exitflag 0
	
	capture confirm file "${VCQI_OUTPUT_FOLDER}/RI_QUAL_01_${ANALYSIS_COUNTER}.dta"
	if _rc != 0 {
		capture confirm file "${VCQI_OUTPUT_FOLDER}/RI_QUAL_01_1.dta"
		if _rc == 0 {
			copy "${VCQI_OUTPUT_FOLDER}/RI_QUAL_01_1.dta" "${VCQI_OUTPUT_FOLDER}/RI_QUAL_01_${ANALYSIS_COUNTER}.dta"
			vcqi_global RI_QUAL_01_TEMP_DATASETS $RI_QUAL_01_TEMP_DATASETS RI_QUAL_01_${ANALYSIS_COUNTER}
			vcqi_log_comment $VCP 2 Warning "${VCQI_OUTPUT_FOLDER}/RI_QUAL_01_${ANALYSIS_COUNTER}.dta does not exist. VCQI will make a copy of RI_QUAL_01_1.dta and proceed."
		}			
		else {
			local exitflag 1 
			di as error  				  "The file ${VCQI_OUTPUT_FOLDER}/RI_QUAL_01_${ANALYSIS_COUNTER}.dta does not exist. Run RI_QUAL_01_03DV"
			vcqi_log_comment $VCP 1 Error "The file ${VCQI_OUTPUT_FOLDER}/RI_QUAL_01_${ANALYSIS_COUNTER}.dta does not exist. Run RI_QUAL_01_03DV"
		}
	}
