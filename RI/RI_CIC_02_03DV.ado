*! RI_CIC_02_03DV version 1.01 - Biostat Global Consulting - 2020-04-22
*******************************************************************************
* Change log
* 				Updated
*				version
...
* 2020-04022	1.01	Dale Rhoda		Allow user to specify CIC_XMAX
*******************************************************************************

program define RI_CIC_02_03DV
	version 14.1
	
	local oldvcp $VCP
	global VCP RI_CIC_02_03DV
		...
		* Create variables for each dose intervals 1-2 from both card and register
		...
		foreach s in card register {	
			foreach d in `=lower("$RI_MULTI_2_DOSE_LIST")' `=lower("$RI_MULTI_3_DOSE_LIST")' {
				...
				global latter_`s'_`d'_1_2 `d'2 // Name of the latter dose; used in 06PO
			}
		}
		...
		* Create variables for each dose intervals 2-3 from both card and register
		foreach s in card register {	
			foreach d in `=lower("$RI_MULTI_3_DOSE_LIST")' {
				...
				global latter_`s'_`d'_2_3 `d'3 // Name of the latter dose; used in 06PO
			}
		}
		...
		* Calculate x-max...
		...
		foreach cr in card register {
			...
			* Now update var based on the data...
			...
			gen xmax_`cr' = $RI_CIC_02_XMAX_INTERVAL * ceil(xmax_temp2_`cr'/$RI_CIC_02_XMAX_INTERVAL) // round up to the nearest xmax_interval (default is 50)
			drop xmax_temp_`cr' xmax_temp2_`cr'
			global XMAX_`cr' = xmax_`cr'[1]
						
			* Allow the user to over-ride the xmax by setting the global CIC_XMAX
			if "$CIC_XMAX" != "" global XMAX_`cr' = $CIC_XMAX			
		}
				
		save "${VCQI_OUTPUT_FOLDER}/RI_CIC_02_${ANALYSIS_COUNTER}", replace
	}
