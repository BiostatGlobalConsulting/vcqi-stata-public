*! SIA_COVG_01_06PO version 1.06 - Biostat Global Consulting - 2017-08-26
*******************************************************************************
* Change log
* 				Updated
*				version
* Date 			number 	Name			What Changed
* 2016-02-17	1.01	MK Trimner		Changed export option in opplot
...
program define SIA_COVG_01_06PO
	version 14.1
	
	local oldvcp $VCP
	global VCP SIA_COVG_01_06PO
		...
		* Make organ pipe plots
		...
		if "$VCQI_MAKE_OP_PLOTS" == "1" {
			...
			* Now make the plots themselves - one for each stratum
			...
			local subtitle SIA_COVG_01: Received Campaign Dose
			...
			forvalues i = 1/`opp_nstrata' {
				...
				if $SAVE_VCQI_GPH_FILES ///
					local savegph   saving("Plots_OP/SIA_COVG_01_${ANALYSIS_COUNTER}_opplot_`opp_stratum_id_`i''_`opp_stratum_name_`i''", replace)

				local savedata
				if $VCQI_SAVE_OP_PLOT_DATA ///
					local savedata savedata(Plots_OP/SIA_COVG_01_${ANALYSIS_COUNTER}_opplot_`opp_stratum_id_`i''_`opp_stratum_name_`i'')

				opplot got_sia_dose  , clustvar(clusterid) plotn  weightvar(psweight) ///
					   ...
					   export(Plots_OP/SIA_COVG_01_${ANALYSIS_COUNTER}_opplot_`opp_stratum_id_`i''_`opp_stratum_name_`i''.png)
					   
				vcqi_log_comment $VCP 3 Comment "Graphic file: SIA_COVG_01_${ANALYSIS_COUNTER}_opplot_`opp_stratum_id_`i''_`opp_stratum_name_`i''.png was created and saved."
		...
		* Make the inchworm plot
		...
		if "$VCQI_MAKE_IW_PLOTS" == "1" {
			...
			* the number to the screen to calibrate the user's expectations
			...
			vcqi_to_iwplot , database(${VCQI_OUTPUT_FOLDER}/SIA_COVG_01_${ANALYSIS_COUNTER}_a_database) ///
				filetag(SIA_COVG_01) ///
				datafile(${VCQI_OUTPUT_FOLDER}/SIA_COVG_01_${ANALYSIS_COUNTER}) ///
				title(SIA - Received SIA Dose (%)) name(SIA_COVG_01_${ANALYSIS_COUNTER}_iwplot)
