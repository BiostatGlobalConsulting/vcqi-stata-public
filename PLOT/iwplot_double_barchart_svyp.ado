*! iwplot_double_barchart_svyp version 1.26 - Biostat Global Consulting - 2021-01-13
*******************************************************************************
* Change log
* 				Updated
*				version
...
*
* 2021-02-01	1.27	Dale Rhoda		Copy iwplot_svyp and make this 
*                                       dedicated program that *ONLY* makes
*                                       double bar charts.  Note that there
*                                       are some superflous options and some
*                                       useless calculations associated with
*                                       inchworm coordinates here; this program
*                                       could be made quite a bit more lean
*                                       and therefore more fast
...
program define iwplot_double_barchart_svyp
	version 14.1
	...
	* write an entry in the log file if we are running VCQI
	...
	`q' {
		...
		* syntax to add bars to the plot
		...
		gen xbar = .
		forvalues i = 1/`bign' {
			replace ybar = rownumber+0.40 			if dn == `i' & inlist(j,1,2) & "`areacolor`i''" != "none"
			replace ybar = rownumber-0.00 			if dn == `i' & inlist(j,1,2) & "`areacolor`i''" == "none"
			replace xbar = `xrangemin' 			    if dn == `i' & j == 1
			replace xbar = `=min(`=table[`i', 1]',100)'	if dn == `i' & j == 2
			if "`areacolor`i''" == "none" local plotit `plotit' ( area ybar xbar if dn == `i', color(`outlinecolor`i'') lwidth(vthin) fcolor(`areacolor`i'')  fi(`areaintensity`i'') base(`=`rownumber`i''-0.40') )
			else                          local plotit `plotit' ( area ybar xbar if dn == `i', color(`outlinecolor`i'') lwidth(vthin) fcolor(`areacolor`i'')  fi(`areaintensity`i'') base(`=`rownumber`i''+0.00') )
		}	
		...
		* or smooth polygons, or both; smooth is the default
		...
		forvalues i = 1/`bign' {
			...
			* Plot the 2-sided CI instead of inchworm shape if user specifies they want to see bars
			...
			if "`areacolor`i''" == "none" local plotit `plotit' (scatteri `=`rownumber`i''-0.2' `lb95`i'' `=`rownumber`i''-0.2' `ub95`i'', c(direct) m(i) lw(*0.95) lc(black))
		...
		* export graph in chosen format
		if `"`export'"'. != "" {
			graph export `export'
			noi di as text "Exported bar chart:"
			noi di `"`export'"'
		}
