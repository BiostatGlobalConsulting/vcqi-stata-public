*! make_unwtd_output_database version 1.04 - Biostat Global Consulting - 2017-01-30
*******************************************************************************
* Change log
* 				Updated
*				version
...
*
*										Added variable labels for level1id, level2id and level3id
...
* 2017-01-30	1.04	Dale Rhoda		Generate LEVEL4 output using
...
program define make_unwtd_output_database
	...
	local oldvcp $VCP
	global VCP make_unwtd_output_database
					...
					if "$VCQI_LEVEL4_SET_VARLIST" != "" & ( "${SHOW_LEVELS_`l'_4_TOGETHER}" == "1"  | ( inlist(`l',2,3) & "$SHOW_LEVELS_2_3_4_TOGETHER" == "1"  )) {
						...
						forvalues j = 1/$LEVEL4_SET_NROWS {
							...
							if "${LEVEL4_SET_ROWTYPE_`j'}" == "DATA_ROW" {
								summarize `variable' if level`l'id == `i' & ${LEVEL4_SET_CONDITION_`j'}	
								if r(N) == 0 post go (`l') (`i') ("`j'") ("`l4name'") ("`variable'") (0) (0)
								if r(N)  > 0 post go (`l') (`i') ("`j'") ("`l4name'") ("`variable'") (r(mean)) (r(N)) 
							}
