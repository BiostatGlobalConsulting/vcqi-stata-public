*! make_TT_augmented_dataset version 1.01 - Biostat Global Consulting - 2018-06-19
*******************************************************************************
* Change log
* 				Updated
*				version
* Date 			number 	Name			What Changed
* 2017-01-10	1.00	MK Trimner		Original version copied from make_TT_augmented_dataset
* 2018-06-19	1.01	?				?
*******************************************************************************

* This program creates one TT dataset containing the original TT dataset provided in VCQI 
* And all TT Indicator datasets found in the OUTPUTPATH. 
...
*
* 1. During TT Control Program by placing the program sytax before vcqi_cleanup. 
...
*
* This program can only be ran on TT Indicators that result in a dataset with a single line per person. 
*
********************************************************************************

program define make_TT_augmented_dataset
	version 14.1
	...
	quietly {
		...
		* Setup global TTLIST to contain all indicators that could have been run...
		* Note: Anytime a new TT indicator is created it will need to be added to this list
		* Note: This program will only work for indicators where the dataset has 1 row per person
				
		global TTLIST TT_COVG_01 
		...
		* Delete dataset if it already existis
		capture confirm file "TT_augmented_dataset.dta"
		if !_rc erase "TT_augmented_dataset.dta"
		
		* First step is to check to see which TT VCQI datasets have been created...
		...
		forvalues i = 1/`=wordcount("`filelist'")' {
			...
			* Save a copy of the dataset file that will be edited with any variable renames
			...
			foreach v of varlist * {
				...
				* Create chars with varname, analysis counter and TT indicator
				char `v'[Varname] `v'
		...
		* Start the dataset with the original TT dataset provided in VCQI
		use "${VCQI_DATA_FOLDER}/${VCQI_TT_DATASET}", clear
		
		save "TT_augmented_dataset", replace

		*Merge together with the first file in TTLIST
		merge 1:1 TT01 TT03 TT11 TT12 using "VCQI_ADS_`=word("`filelist'",1)'", update nogen
			...
			foreach u in `VARLIST_`fn'' {
				if strpos("`VARLIST'","`u' ") >= 1 {
					...
					* Create two small datasets with specified variable and the identifiers to compare values
					use "TT_augmented_dataset", clear
					keep respid clusterid stratumid `u'			
					
					* Save as a new name
					save "TT_augmented_dataset_`i'_`u'", replace
					...
					* Merge the two datasets together
					merge 1:1 respid clusterid stratumid using "TT_augmented_dataset_`i'_`u'"
					...
					* If l is set to erase comparison datasets, erase them...
					if "`erase'" == "" {
						erase "TT_augmented_dataset_`i'_`u'.dta" 
	...
	di as text "VCQI Augmented Dataset was saved as TT_augmented_dataset.dta".
